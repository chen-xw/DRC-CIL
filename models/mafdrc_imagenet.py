...
from utils.inc_net import MAFDRC_ImageNet
...
class Mafdrc_imagenet(BaseLearner):

    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._snet = None
        self._network = MAFDRC_ImageNet(args['convnet_type'], False,scale=1)
    ...
    def _model_fusion(self, test_loader): 
        ...
        self._snet = MAFDRC_ImageNet(self.args['convnet_type'], False,scale=1)
        ...
        if len(self._multiple_gpus) > 1:
            self._snet.module.convnet.load_state_dict(copy.deepcopy(self._old_network.module.convnet.state_dict()))
            self._snet.module.copy_fc(copy.deepcopy(self._old_network.module.fc))
        else:
            self._snet.convnet.load_state_dict(copy.deepcopy(self._old_network.convnet.state_dict()))
            self._snet.copy_fc(copy.deepcopy(self._old_network.fc))
...
def Logit_adjustment(self,train_loader,tro):
    ...
    "tro 1.6 for ImageNet100 B0 20 steps"
